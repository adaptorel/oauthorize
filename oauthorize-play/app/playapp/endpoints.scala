...
class AccessTokenEndpointPlay(
  config: Oauth2Config,
  logger: Logging,
  processor: AccessTokenEndpoint,
  tenantResolver: TenantResolver) extends Oauth2BodyReaderFilter(logger) {
  ...
  override def bodyProcessor(oauthRequest: OauthRequest, req: RequestHeader)(implicit ctx: ExecutionContext) = {
    implicit val tenant = tenantResolver.resolveTenant(req)
...
class RefreshTokenEndpointPlay(
  config: Oauth2Config,
  logger: Logging,
  processor: RefreshTokenEndpoint,
  tenantResolver: TenantResolver) extends Oauth2BodyReaderFilter(logger) {
...
class ClientCredentialsGrantPlay(
  config: Oauth2Config,
  logger: Logging,
  processor: ClientCredentialsGrant,
  tenantResolver: TenantResolver) extends Oauth2BodyReaderFilter(logger) {
...
class ResourceOwnerCredentialsGrantPlay(
  config: Oauth2Config,
  logger: Logging,
  processor: ResourceOwnerCredentialsGrant,
  tenantResolver: TenantResolver) extends Oauth2BodyReaderFilter(logger) {
...
class ImplicitGrantPlay(
  config: Oauth2Config,
  logger: Logging,
  processor: ImplicitGrant,
  tenantResolver: TenantResolver) extends Oauth2BodyReaderFilter(logger) with securesocial.core.SecureSocial {
...
class UserApprovalPlay(
  ...
  logger: Logging,
  store: Oauth2Store,
  processor: UserApproval,
  tenantResolver: TenantResolver) extends Oauth2BodyReaderFilter(logger) with SecureSocial {
  ...
  private def lazyProcessApprove(
    a: OauthRequest,
    u: Oauth2User,
    req: RequestHeader)(implicit ctx: ExecutionContext, tenant: Tenant): Future[SimpleResult] = {
