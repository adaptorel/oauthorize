...
object AuthorizationCodeGrant extends AuthorizationCodePlay(config, logger, authzCodeEndpoint, tenantResolver)
object ImplicitGrant extends ImplicitGrantPlay(config, logger, implicitGrantEndpoint, tenantResolver)
object ClientCredentialsGrant extends ClientCredentialsGrantPlay(config, logger, clientCredentialsEndpoint, tenantResolver)
object AccessTokenEndpoint extends AccessTokenEndpointPlay(config, logger, accessTokenEnpoint, tenantResolver)
object RefreshTokenEndpoint extends RefreshTokenEndpointPlay(config, logger, refreshTokenEndpoint, tenantResolver)
object ResourceOwnerCredentialsGrant extends ResourceOwnerCredentialsGrantPlay(config, logger, resourceOwnerEndpoint, tenantResolver)
object UserApprovalEndpoint extends UserApprovalPlay(config, logger, store, userApprovalEndpoint, tenantResolver)
...
object oauth2provider {
  ...
  val tenantResolver = new DefaultTenantResolver
